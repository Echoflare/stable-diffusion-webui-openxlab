...
fooocus_repo = 'lllyasviel/Fooocus'
rename_repo = 'Fooocus'
git_url = 'gitcode.com'
fooocus_port = 7860
...
def run_fooocus():
    ...
    if not os.path.exists(f'{install_path}/{rename_repo}'):
        os.system(f"git clone https://{git_url}/{fooocus_repo} {install_path}/{rename_repo}")
        if not os.path.exists(f'{install_path}/{rename_repo}'):
            print(f'在克隆 https://{git_url}/{fooocus_repo} 时出错')
            exit()

    model_download(sd_models, 'models/checkpoints')
    model_download(lora_models, 'models/loras')
    model_download(vae_models, 'models/vae')
    if ControlNet:
        model_download(controlnet_models, 'models/controlnet')
    model_download(hypernetwork_models, 'models/hypernetworks')
    model_download(embedding_models, 'models/embeddings')
    model_download(esrgan_models, 'models/upscale_models')
    ...
    os.chdir(f"{install_path}/{rename_repo}")
    os.system(f"python launch.py {' '.join(fooocus_args)} --port {fooocus_port}")

# 启动
import threading

if __name__ == '__main__':
    fooocus = threading.Thread(target=run_fooocus)
